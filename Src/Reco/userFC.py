...
def calcNearestNeighbor(userid,users_dic,item_dic):
    ...
    for neighbor in neighbors:
        dist=calcCosDist(users_dic[userid],users_dic[neighbor])  #calcSimlaryCosDist  calcCosDist
...
def recommendByUserFC(file_name,userid,k=5):
    ...
    for neighbor in neighbors:
        ...
        for movie in movies:
            if movie[0] not in recommend_list and movie[0] not in user_movies: 
                recommend_list.append(movie[0])
    
    return recommend_list,user_movies,test_item_to_user,neighbors
...
if __name__ == '__main__':
    ...
    recommend_list,user_movie,items_movie,neighbors=recommendByUserFC("/Users/wuyinghao/Downloads/ml-100k/u.data",293,20)
    ...
    for movie_id in recommend_list[:10]:
        from_user=[]
